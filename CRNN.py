...
from .CNNFDY import CNN
from conformermain.conformer import Conformer
from models.conformer.conformer_encoder import ConformerEncoder
from recipes.dcase2023_task4_baseline.local.beats.backbone import TransformerEncoder
from recipes.dcase2023_task4_baseline.local.beats.Tokenizers import TokenizersConfig
from recipes.dcase2023_task4_baseline.STFT_Transformer.stft_transformer_final import BirdModel
...
class CRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #
        # encoder_layer = nn.TransformerEncoderLayer(d_model=156, nhead=78)
        # encoder_layer2 = nn.TransformerEncoderLayer(d_model=256, nhead=256)
        # self.rnn= nn.TransformerEncoder(encoder_layer2, num_layers=5)
        # self.rnn3 = nn.TransformerEncoder(encoder_layer, num_layers=2)

        #self.rnn = TransformerEncoder(TokenizersConfig)

        # , norm=nn.LayerNorm([128, 250]))
        # self.avgpool = nn.AvgPool3d((1, 1, 4))
        #self.rnn = Conformer(num_classes=10, input_dim=896,encoder_dim=32, num_encoder_layers=3)
        # self.rnn = FocalNet(in_chans=1)
        #self.rnn = ConformerEncoder(idim=256)
        # print(dropout)
        # print(dropout_recurrent)
        #self.resape= torch.nn.functional.adaptive_avg_pool1d(embeddings, x.shape[1]).transpose(1, 2)
        #self.rnn = BirdModel(backbone="deit_base_distilled_patch16_384",out_dim=512)

        #self.densex = nn.Linear(578, 156)
    ...
    def forward(self, x, pad_mask=None, embeddings=None):
        ...
        #print(x.shape)
        #x = torch.nn.functional.adaptive_avg_pool2d(x,(384,384))
        #print(x.shape)
        #exit()
        x = self.rnn(x)
        ...
        #x= x['logits']
        #exit()
        #x = self.densex(x.permute(0,2,1)).permute(0,2,1)
        # x = x.permute(0, 2, 1)
        # x = self.rnn3(x)
        # x = x.permute(0, 2, 1)
        x = self.dropout(x)
